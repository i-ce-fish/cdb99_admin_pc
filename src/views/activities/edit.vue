...
import { getActivity, putActivity } from "../../api/activity"
...
export default {
  ...
  methods: {
    async get() {
      const response = await getActivity(this.$route.query.id)
      this.activityForm = response.data
    },

    async api() {
      const res = await putActivity(this.$router.query.id, this.activityForm)
      // if (res.code === '200') {
