...
import { putOutfit, getOutfit } from "../../api/outfit"
...
export default {
  ...
  methods: {
    async get() {
      const res = await getOutfit(this.$route.query.id)
      this.outfitForm = res.data
    },

    async api() {
      const res = await putOutfit(this.$route.query.id, this.outfitForm)
